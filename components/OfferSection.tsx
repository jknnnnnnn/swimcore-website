...
const offer: OfferItem[] = [
  {
    ...
    icon: "baby",
    label: "Start",
    accent: "from-orange/10 to-card",
    iconStyle: "bg-orange text-ink",
  },
  {
...
export function OfferSection() {
  return (
    <section id="oferta" className="scroll-mt-16 bg-ink py-28 sm:py-32 lg:py-40">
      <Container>
        <SectionHeading
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:mt-16 lg:grid-cols-3">
          {offer.map((item, index) => (
            <article
              key={item.title}
              className={`group relative min-h-[250px] overflow-hidden rounded-[1.75rem] border border-line bg-gradient-to-br ${item.accent} p-6 shadow-none backdrop-blur-xl transition-all duration-500 hover:-translate-y-2 hover:border-orange/45 hover:shadow-glow sm:p-7`}
            >
              <div className="absolute -right-10 -top-12 h-32 w-32 rounded-full border-[18px] border-white/5 transition-transform duration-500 group-hover:scale-110" />
              <div className="relative flex items-start justify-between gap-4">
                <span className={`grid h-12 w-12 place-items-center rounded-2xl shadow-card ${item.iconStyle}`}>
                  <Icon name={item.icon} className="h-6 w-6" />
                </span>
                <span className="text-sm font-black tracking-[-0.04em] text-white/10">
                  0{index + 1}
                </span>
              </div>
