...
export function FAQSection() {
  return (
    <section id="faq" className="scroll-mt-16 bg-navy py-20 sm:py-24 lg:py-28">
      <Container>
        <SectionHeading
          eyebrow="FAQ"
        ...
        <div className="mx-auto mt-10 grid max-w-4xl gap-3 lg:mt-14">
          {questions.map((item) => (
            <details
              key={item.question}
              className="group overflow-hidden rounded-2xl border border-white/10 bg-white/[0.05] shadow-card backdrop-blur-xl transition-all duration-300 open:border-cyan/40 open:bg-white/[0.08] open:shadow-glow"
            >
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-5 text-left text-sm font-black leading-6 text-white transition-colors duration-300 hover:text-cyan focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan sm:px-6 sm:text-base [&::-webkit-details-marker]:hidden">
                {item.question}
                <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full border border-white/15 bg-white/[0.06] text-xl font-light text-cyan transition-transform duration-300 group-open:rotate-45">
                  +
                </span>
              </summary>
              <p className="border-t border-white/10 px-5 py-5 text-sm leading-7 text-muted sm:px-6">
                {item.answer}
              </p>
            </details>
          ))}
